        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header text-white bg-info">
                    <h4><i class="fas fa-clipboard-list"></i> {{ trans('action.upload.upload').' '.trans('action.search') }}</h4>
                </div>
                <div class="card-body">
                	<form action="{{ route('upload.search') }}" method="post">
                        ...
                        <ul class="list-inline">
                            ...
                            <li class="list-inline-item">{{ App\Button::To(false,route('upload.index'),trans('action.reset'),null,'btn-secondary','undo') }}</li>
                        </ul>
                        {{-- 篩選器設定 --}}
					...
					<div class="table-responsive">
                        <table id="data" class="table table-hover table-bordered text-center">
                            ...
                            <tbody>
                                @foreach ($uploads_search as $upload)
                                    <tr>
                                        <td>{{ $upload->id }}</td>
                ...
                <div class="card-footer pagination justify-content-center table-responsive">
					{!! $uploads_search->links("pagination::bootstrap-4") !!}
				</div>
            </div>
        </div>
    </div>
